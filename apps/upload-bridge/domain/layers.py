...
class LayerManager(QObject):
    ...
    def _overwrite_pixels(self, bottom: List[Color], top: List[Color], top_alpha: Optional[List[int]] = None) -> List[Color]:
        """
        Order-only overwrite: higher layer pixels override lower layers based on alpha channel.
        Pixels with alpha=0 are transparent (let lower layer show through).

        This is the core compositing logic for order-only rendering - no blend modes,
        no opacity math, just simple overwrite based on alpha channel.

        Args:
            bottom: Bottom layer pixels (will be overwritten)
            top: Top layer pixels (will overwrite bottom)
            top_alpha: Per-pixel alpha values for top layer (0-255). None = all opaque (255).

        Returns:
            Result pixels with top layer overwriting bottom (alpha=0 pixels are transparent)
        """
        result = list(bottom)
        for i in range(min(len(top), len(result))):
            ...
            if alpha > 0:
                result[i] = top[i]
            # Alpha = 0 means transparent - keep bottom layer pixel
        return result
    ...
    def _apply_layer_automation(
        ...
        if track.automation is None or len(track.automation) == 0:
            # No automation actions - check for old animation system fallback
            if LayerAction is None:
            ...
            # No automation and no old animation - return pixels unchanged
            return pixels
