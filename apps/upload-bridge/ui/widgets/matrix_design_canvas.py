...
from core.gradient import Gradient, GradientType, GradientStop, GradientRenderer, create_simple_gradient, PRESET_GRADIENTS
...
class MatrixDesignCanvas(QWidget):
    ...
    color_picked = Signal(tuple)  # (r, g, b) emitted when eyedropper picks a color
    ...
    def __init__(
        ...
        self._gradient_press_pos: Optional[QPoint] = None  # Track mouse press position for drag detection
        ...
        self._bucket_fill_contiguous: bool = True  # Contiguous fill vs global replacement
        ...
        self._selection_buffer: Optional[List[List[RGB]]] = None  # Copied pixels
        self._selection_moving: bool = False
        self._selection_drag_start: Optional[QPoint] = None
        self._selection_origin: Optional[Tuple[int, int]] = None
        self._marching_ants_timer: Optional[QTimer] = None
        self._marching_ants_offset: int = 0
        
        # Gradient tool state
        self._gradient_type: GradientType = GradientType.LINEAR
        self._gradient: Optional[Gradient] = None
        self._gradient_start_point: Optional[QPoint] = None
        self._gradient_end_point: Optional[QPoint] = None
        self._gradient_preview_mode: bool = False
        self._gradient_dragging_handle: Optional[str] = None  # "start" or "end"
        self._gradient_press_pos: Optional[QPoint] = None
    ...
    def set_drawing_mode(self, mode: DrawingMode):
        ...
        if self._drawing_mode == DrawingMode.SELECTION and mode != DrawingMode.SELECTION:
        ...
        if self._drawing_mode == DrawingMode.EYEDROPPER:
            self.setCursor(Qt.CrossCursor)  # Should ideally be a pipette icon
        else:
    ...
    def set_bucket_fill_contiguous(self, enabled: bool) -> None:
        """Set whether bucket fill should be contiguous (flood fill) or global."""
        self._bucket_fill_contiguous = bool(enabled)

    def get_bucket_fill_contiguous(self) -> bool:
        """Check if bucket fill is contiguous."""
        return self._bucket_fill_contiguous
    ...
    # Mouse handling
    # ------------------------------------------------------------------
    def _apply_shift_constraints(self, start_pos: Tuple[int, int], end_pos: Tuple[int, int]) -> Tuple[int, int]:
        """Apply Shift key constraints to shape coordinates."""
        x1, y1 = start_pos
        x2, y2 = end_pos
        dx = x2 - x1
        dy = y2 - y1
        
        if self._drawing_mode == DrawingMode.LINE:
            ...
            if abs(dx) < abs(dy) / 2:
                ...
                return x1 + dist * sign_x, y1 + dist * sign_y
        elif self._drawing_mode in (DrawingMode.RECTANGLE, DrawingMode.CIRCLE):
            # Square / Circle constraint
            size = max(abs(dx), abs(dy))
            sign_x = 1 if dx >= 0 else -1
            sign_y = 1 if dy >= 0 else -1
            return x1 + size * sign_x, y1 + size * sign_y
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() in (Qt.LeftButton, Qt.RightButton):
            if event.button() == Qt.LeftButton:
                # Handle gradient tool - allow both brush mode and drag-to-define mode
                if self._drawing_mode == DrawingMode.GRADIENT:
                    pos = event.position().toPoint()
                    # Check if clicking on existing gradient handles (for editing existing gradient)
                    if self._gradient_start_point and self._gradient_end_point:
                    ...
                    # For gradient tool: store press position to detect drag
                    # Allow normal painting with gradient brush cycling on click
                    # Enter preview mode only if user drags
                    self._gradient_press_pos = pos
                    self._gradient_step_index = 0
                    # Don't enter preview mode yet - allow normal painting
                    # Preview mode will be activated in mouseMoveEvent if user drags
            ...
            if 0 <= x < self._matrix_width and 0 <= y < self._matrix_height:
                ...
                if self._drawing_mode == DrawingMode.BUCKET_FILL:
                    ...
                    if self._bucket_fill_contiguous:
                        ...
                        if not colors_match(target_color, fill_color, tolerance):
                            for py in range(self._matrix_height):
                                for px in range(self._matrix_width):
                                    if colors_match(self._grid[py][px], target_color, tolerance):
                                        # Check if cell is active (for irregular shapes)
                                        if self._pattern_metadata and self._pattern_metadata.irregular_shape_enabled:
                                            if not IrregularShapeMapper.is_cell_active(px, py, self._pattern_metadata):
                                                continue
                                        
                                        self._grid[py][px] = fill_color
                                        filled_pixels.append((px, py, fill_color))
                ...
                elif self._drawing_mode == DrawingMode.SELECTION:
                    ...
                    if self._selection_rect:
                        ...
                        if sx <= x < sx + sw and sy <= y < sy + sh:
                            ...
                            self._selection_moving = True
                            self._selection_drag_start = event.position().toPoint()
                            self._selection_origin = (sx, sy)
                            if not self._selection_buffer:
                                self._copy_selection_to_buffer()
                            return

                    # Start new selection (commit old if any)
                    self._commit_selection()
                    self._is_dragging = True
                    self._shape_start = (x, y)
                    self._shape_end = (x, y)
                    self._selection_rect = (x, y, 1, 1)
                    self._start_marching_ants()
                    self.update()
                elif self._drawing_mode in (DrawingMode.PIXEL, DrawingMode.ERASER):
                    self._is_dragging = True
    ...
    def mouseMoveEvent(self, event):
        ...
        if self._is_dragging:
            if self._drawing_mode in (DrawingMode.PIXEL, DrawingMode.ERASER):
                ...
                # Check if we should enter preview mode (drag-to-define)
                if not self._gradient_preview_mode and self._gradient_press_pos:
                    # Check if user has dragged enough to enter preview mode
                    drag_threshold = 5  # pixels
                    drag_dist = ((pos.x() - self._gradient_press_pos.x()) ** 2 + 
                                (pos.y() - self._gradient_press_pos.y()) ** 2) ** 0.5
                    if drag_dist > drag_threshold:
                        # Enter preview mode for drag-to-define gradient
                        self._gradient_start_point = self._gradient_press_pos
                ...
                if self._gradient_preview_mode:
                    # Update gradient end point during drag-to-define
                    ...
                    # Normal painting with gradient brush cycling
                    if event.buttons() & (Qt.LeftButton | Qt.RightButton):
                        button = Qt.LeftButton if event.buttons() & Qt.LeftButton else Qt.RightButton
                        self._handle_paint_event(pos, button)
            elif self._drawing_mode == DrawingMode.SELECTION:
                if self._selection_moving and self._selection_drag_start and self._selection_origin:
                    # Move Selection
                    dx = (pos.x() - self._selection_drag_start.x()) // self._pixel_size
                    dy = (pos.y() - self._selection_drag_start.y()) // self._pixel_size
                    ox, oy = self._selection_origin
                    
                    if self._selection_rect:
                        _, _, w, h = self._selection_rect
                        self._selection_rect = (ox + dx, oy + dy, w, h)
                        self.update()
                else:
                    # Create Selection Rect
                    x, y = cell
                    # Clamp to bounds for selection creation? Standard is yes.
                    x = max(0, min(x, self._matrix_width - 1))
                    y = max(0, min(y, self._matrix_height - 1))
                    ...
                    if self._shape_start:
                        ...
                        self._selection_rect = (x1, y1, x2 - x1 + 1, y2 - y1 + 1)
                        self.update()
            else:
                # Update shape preview
                x, y = cell
                # Allow dragging outside bounds - clamp later if needed, but for now strict bound check
                if 0 <= x < self._matrix_width and 0 <= y < self._matrix_height:
                    target_pos = (x, y)
                    if event.modifiers() & Qt.ShiftModifier and self._shape_start:
                        target_pos = self._apply_shift_constraints(self._shape_start, target_pos)
                    
                    self._shape_end = target_pos
    ...
    def mouseReleaseEvent(self, event):
        ...
        if self._drawing_mode == DrawingMode.GRADIENT:
            if self._gradient_preview_mode:
                ...
                if was_dragging:
                    self.painting_finished.emit()
                self._gradient_press_pos = None
                self._is_dragging = False
                return
            else:
                # Normal click painting - already handled, just clean up
                self._gradient_press_pos = None
        
        if self._is_dragging:
            if self._drawing_mode == DrawingMode.SELECTION:
                # Selection finished (creation or move)
                self._selection_moving = False
                pass
        ...
        if was_dragging and self._drawing_mode not in (DrawingMode.SELECTION, DrawingMode.GRADIENT):
            self.painting_finished.emit()
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == Qt.Key_Escape:
            if self._selection_rect or self._selection_buffer:
                self._commit_selection()
            self._drawing_mode = DrawingMode.PIXEL
            self.setCursor(Qt.ArrowCursor)
            self.update()
            ...
            if self._drawing_mode == DrawingMode.SELECTION and self._selection_rect:
                # Fill selection with erase color (Delete)
                # If we have a buffer (floating), just kill it? 
                # Standard behavior: Delete fills area with background/erase color.
                if self._selection_moving and self._selection_buffer:
                    # Logic: We are holding content. Delete destroys it?
                    ...
                    for dy in range(h):
                        for dx in range(w):
                            ...
                            if 0 <= px < self._matrix_width and 0 <= py < self._matrix_height:
                                self._grid[py][px] = self._erase_color
                                self.pixel_updated.emit(px, py, self._erase_color)
                    self._dirty_regions.append((x, y, w, h))
                    self.update()
            event.accept()
            return
            ...
            if self._drawing_mode == DrawingMode.SELECTION and self._selection_rect:
                self._copy_to_clipboard()
                # Erase source
                if not self._selection_moving: # If already floating, nothing to erase (already erased at start of move)
                    self._erase_selection_area()
                event.accept()
                return
        ...
        elif ctrl and event.key() == Qt.Key_0:
            # Reset zoom
            self.reset_zoom()
            event.accept()
            return
        elif ctrl and event.key() == Qt.Key_1:
            # Fit to window
            self._fit_to_window()
            event.accept()
            return
    ...
    def _draw_selection(self, painter: QPainter):
        if self._selection_buffer and self._selection_rect:
            bx, by, bw, bh = self._selection_rect
            for y in range(bh):
                for x in range(bw):
                    if 0 <= y < len(self._selection_buffer) and 0 <= x < len(self._selection_buffer[y]):
                        color = self._selection_buffer[y][x]
                        screen_x = (bx + x) * self._pixel_size + 1
                        screen_y = (by + y) * self._pixel_size + 1
                        screen_w = self._pixel_size - 2
                        screen_h = self._pixel_size - 2
                        self._draw_pixel_tile(painter, screen_x, screen_y, screen_w, screen_h, color)
        
        if self._selection_rect:
            x, y, w, h = self._selection_rect
            rect_x = x * self._pixel_size
            rect_y = y * self._pixel_size
            rect_w = w * self._pixel_size
            rect_h = h * self._pixel_size
            
            painter.setPen(QPen(Qt.black, 1, Qt.SolidLine))
            painter.setBrush(Qt.NoBrush)
            painter.drawRect(rect_x - 1, rect_y - 1, rect_w + 2, rect_h + 2)
    ...
    def _start_marching_ants(self):
        if not self._ants_timer:
            self._ants_timer = QTimer(self)
            self._ants_timer.timeout.connect(self._animate_ants)
        if not self._ants_timer.isActive():
            self._ants_timer.start(100)
        ...
        if self._selection_buffer and self._selection_rect:
            ...
            for y in range(bh):
                for x in range(bw):
                    ...
                    if 0 <= px < self._matrix_width and 0 <= py < self._matrix_height:
                         if 0 <= y < len(self._selection_buffer) and 0 <= x < len(self._selection_buffer[y]):
                            self._grid[py][px] = self._selection_buffer[y][x]
                            self.pixel_updated.emit(px, py, self._grid[py][px])
        
        if self._selection_buffer:
            self._full_repaint_needed = True

        self._selection_rect = None
        self._selection_buffer = None
        self._selection_moving = False
        if self._ants_timer: self._ants_timer.stop()
        self.update()

    def _copy_selection_to_buffer(self):
        if not self._selection_rect: return
        x, y, w, h = self._selection_rect
        ...
        for dy in range(h):
            ...
            for dx in range(w):
                px, py = x + dx, y + dy
                valid = 0 <= px < self._matrix_width and 0 <= py < self._matrix_height
                val = self._grid[py][px] if valid else self._erase_color
                row.append(val)
                if valid:
                    self._grid[py][px] = self._erase_color
                    self.pixel_updated.emit(px, py, self._erase_color)
            self._selection_buffer.append(row)
        if self._selection_rect:
            self._dirty_regions.append(self._selection_rect)
        self.update()

    # ------------------------------------------------------------------
    # Gradient Tool
    # ------------------------------------------------------------------
    def set_gradient_type(self, gradient_type: GradientType) -> None:
        """Set gradient type (linear or radial)."""
        self._gradient_type = gradient_type
    
    def set_gradient_colors(self, color_start: RGB, color_end: RGB) -> None:
        """Set simple 2-stop gradient colors."""
        self._gradient = create_simple_gradient(color_start, color_end, self._gradient_type)
    
    def set_gradient(self, gradient: Gradient) -> None:
        """Set a custom multi-stop gradient."""
        self._gradient = gradient
        self._gradient_type = gradient.type
    
    def get_gradient(self) -> Optional[Gradient]:
        """Get current gradient."""
        return self._gradient
    
    def load_preset_gradient(self, preset_name: str) -> bool:
        """Load a preset gradient by name."""
        if preset_name in PRESET_GRADIENTS:
            self._gradient = PRESET_GRADIENTS[preset_name]
            self._gradient_type = self._gradient.type
            return True
        return False
    
    def _apply_gradient_interactive(self) -> None:
        """Apply gradient based on drag-to-define points."""
        ...
        if not self._gradient:
            self._gradient = create_simple_gradient(
                self._current_color,
                self._erase_color,
                self._gradient_type
            )
        
        # Convert QPoint to pixel coordinates
        start_cell = self._point_to_cell(self._gradient_start_point)
        end_cell = self._point_to_cell(self._gradient_end_point)
        ...
        # Render gradient to grid
        if self._gradient_type == GradientType.LINEAR:
            grid = GradientRenderer.render_linear(
                self._gradient,
                self._matrix_width,
                self._matrix_height,
                start_x, start_y,
            ...
            grid = GradientRenderer.render_radial(
                self._gradient,
                self._matrix_width,
                self._matrix_height,
                start_x, start_y,
        ...
        for y in range(self._matrix_height):
            for x in range(self._matrix_width):
                ...
                self.pixel_updated.emit(x, y, grid[y][x])
        
        self._full_repaint_needed = True
        self.update()
    
    def _draw_gradient_preview(self, painter: QPainter) -> None:
        """Draw gradient preview along the line."""
        if not self._gradient_start_point or not self._gradient_end_point:
            return
        
        # Create a small preview gradient
        preview_width = 100
        preview_height = 10
        ...
        # Render preview
        if self._gradient_type == GradientType.LINEAR:
            preview_grid = GradientRenderer.render_linear(
                self._gradient,
                preview_width,
                preview_height,
                0, preview_height / 2,
            ...
            preview_grid = GradientRenderer.render_radial(
                self._gradient,
                preview_width,
                preview_height,
                preview_width / 2,
        ...
        # Draw preview as a small bar near the gradient line
        mid_x = (self._gradient_start_point.x() + self._gradient_end_point.x()) // 2
        mid_y = (self._gradient_start_point.y() + self._gradient_end_point.y()) // 2
        
        for x in range(preview_width):
            color = preview_grid[preview_height // 2][x]
            painter.setPen(QColor(*color))
            painter.drawLine(
                mid_x - preview_width // 2 + x, mid_y - 5,
                mid_x - preview_width // 2 + x, mid_y + 5
            )
    ...
    # Clipboard
    # ------------------------------------------------------------------
    def _erase_selection_area(self):
        """Erase the pixels under the current selection rect."""
        if not self._selection_rect: return
        x, y, w, h = self._selection_rect
        for dy in range(h):
            for dx in range(w):
    ...
    def _copy_to_clipboard(self):
        ...
        # If we have a moving buffer, copy that. Otherwise copy from grid.
        if self._selection_moving and self._selection_buffer:
            data = self._selection_buffer
        else:
            for dy in range(h):
                row = []
                for dx in range(w):
                    ...
                    if 0 <= px < self._matrix_width and 0 <= py < self._matrix_height:
                        ...
                        row.append(self._erase_color)
                data.append(row)
                
        # Serialize
        clipboard_data = {
            "type": "pixel_led_pattern_selection",
        ...
        try:
            json_str = json.dumps(clipboard_data)
    ...
    def _paste_from_clipboard(self):
        ...
        try:
            ...
            if not isinstance(data, dict) or data.get("type") != "pixel_led_pattern_selection":
            ...
            self.set_drawing_mode(DrawingMode.SELECTION)
            
            # Place in center of screen
            # Calculate visible center based on scroll/pan if possible, otherwise matrix center
            center_x = self._matrix_width // 2
            center_y = self._matrix_height // 2
            
            start_x = center_x - (width // 2)
            start_y = center_y - (height // 2)
            
            self._selection_rect = (start_x, start_y, width, height)
            self._selection_buffer = pixels
            self._selection_moving = True
    ...
    def _handle_paint_event(self, point: QPoint, button: Qt.MouseButton):
        ...
        if button == Qt.RightButton:
            ...
            elif self._drawing_mode == DrawingMode.ERASER:
                color = self._erase_color
            else:
