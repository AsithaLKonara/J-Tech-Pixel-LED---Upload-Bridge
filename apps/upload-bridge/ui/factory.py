"""
Editor Controller Factory - Upload Bridge Application
Provides a factory function for creating the EditorController.
"""
...
from PySide6.QtWidgets import (QMainWindow, QTabWidget, QMenuBar, QMenu,
                                ...
                                QPushButton, QLabel, QWidget, QComboBox, QInputDialog)
...
def create_editor_controller():
    """Factory function to create and return an EditorController instance."""
    # All PySide6 imports are expected to be available by now
    return EditorController()


class EditorController(QMainWindow):
    """
    Main application window for Upload Bridge, now functioning as the EditorController.
    ...
    project_changed = Signal(object) # New signal for project changes from EditorController
    ...
    def __init__(self):
        ...
        # Temporary backward compatibility attributes (will be removed after full migration to repository)
        self.pattern: Pattern = None
        self.current_file: str = None
        self.is_dirty: bool = False
        self._current_project_file: str = None # Added for tracking current project file
        self.project = None # Placeholder for project object if using new architecture
    ...
    def load_project(self, file_path: str) -> bool:
        """Load a project file into the editor controller."""
        try:
            from core.project.project_file import load_project as load_project_file
            from pathlib import Path
            
            pattern, metadata = load_project_file(Path(file_path))
            
            self._current_project_file = file_path
            self.load_pattern_to_all_tabs(pattern, file_path)
            
            # Store metadata in project object for compatibility
            if not hasattr(self, 'project') or self.project is None:
                class ProjectWrapper:
                    def __init__(self, metadata, pattern):
                        self.metadata = metadata
                        self.timeline = type('Timeline', (object,), {
                            'total_frames': metadata.total_frames if hasattr(metadata, 'total_frames') else pattern.frame_count
                        })()
                self.project = ProjectWrapper(metadata, pattern)
            else:
                # Update existing project object
                self.project.metadata = metadata
                if hasattr(self.project, 'timeline'):
                    total_frames = metadata.total_frames if hasattr(metadata, 'total_frames') else pattern.frame_count
                    self.project.timeline.total_frames = total_frames
            
            self.project_changed.emit(self.project)
            
            base_name = Path(file_path).name
            self.setWindowTitle(f"Upload Bridge - {base_name}")
            self.status_bar.showMessage(f"Loaded project: {base_name}")
            
            return True
        except Exception as e:
            logging.getLogger(__name__).error(f"Failed to load project {file_path}: {e}", exc_info=True)
            QMessageBox.critical(self, "Load Error", f"Failed to load project:\n\n{str(e)}")
            self.status_bar.showMessage(f"Failed to load project: {Path(file_path).name}")
            return False

    def save_project(self, file_path: str) -> bool:
        """Save the current project to a file."""
        try:
            from core.project.project_file import save_project as save_project_file
            from pathlib import Path
            ...
            save_project_file(pattern, file_path_obj, use_rle=True)
            ...
            self.setWindowTitle(f"Upload Bridge - {base_name}")
            self.status_bar.showMessage(f"Saved project: {base_name}")
            ...
            logging.getLogger(__name__).error(f"Failed to save project {file_path}: {e}", exc_info=True)
            QMessageBox.critical(self, "Save Error", f"Failed to save project:\n\n{str(e)}")
            self.status_bar.showMessage(f"Failed to save project: {Path(file_path).name}")
            return False
    ...
    def on_tab_changed(self, index: int):
        ...
        if 0 <= index < len(tab_names):
            ...
            if not self._tabs_initialized[tab_name]:
                # #region agent log
                try:
                    debug_log("factory.py:246", "Initializing tab", {"tab_name": tab_name}, hypothesis_id="E")
                except Exception:
                    pass
                # #endregion
                ...
                # #region agent log
                try:
                    debug_log("factory.py:250", "Tab initialization completed", {"tab_name": tab_name}, hypothesis_id="E")
                except Exception:
                    pass
                # #endregion
    ...
    def initialize_tab(self, tab_name: str):
        ...
        try:
            ...
            if tab_name == 'media_upload' and self.media_upload_tab is None:
                ...
                # Connect pattern modification signal
                ...
                # Connect to main window signals for file operations
    ...
    def _connect_playback_signals(self):
        ...
        if self.preview_tab and self.design_tab:
            ...
            if not hasattr(self, '_playback_signals_connected'):
                ...
                # Connect pattern_modified for real-time preview sync
                self.design_tab.pattern_modified.connect(self._on_design_pattern_modified)
    ...
    def _on_design_pattern_modified(self):
        ...
        if self.design_tab and self.preview_tab:
            ...
            if pattern:
                # Update preview tab pattern (debounced)
                if not hasattr(self, '_pattern_sync_timer'):
                    from PySide6.QtCore import QTimer
                    self._pattern_sync_timer = QTimer()
    ...
    def get_tab(self, tab_name: str):
        ...
        tab_map = {
            'media_upload': ('media_upload_tab', 'media_upload'),
            'design_tools': ('design_tab', 'design_tools'),
            'preview': ('preview_tab', 'preview'),
            'flash': ('flash_tab', 'flash'),
            'batch_flash': ('batch_flash_tab', 'batch_flash'),
            'pattern_library': ('pattern_library_tab', 'pattern_library'),
            'audio_reactive': ('audio_reactive_tab', 'audio_reactive'),
            'wifi_upload': ('wifi_upload_tab', 'wifi_upload'),
            'arduino_ide': ('arduino_ide_tab', 'arduino_ide')
        }
        
        if tab_name in tab_map:
            attr_name, key = tab_map[tab_name]
            tab = getattr(self, attr_name, None)
            
            if tab is None or not self._tabs_initialized[key]:
                self.initialize_tab(key)
                # Re-read the tab attribute after initialization
                tab = getattr(self, attr_name, None)
    ...
    def create_menus(self):
        ...
        save_project_action.triggered.connect(self.save_project_menu_action)
        ...
        diagnostic_action = QAction("Export &Diagnostic Report...", self)
        diagnostic_action.triggered.connect(self.show_diagnostic_report)
    ...
    def open_pattern(self):
        ...
        try:
            ...
            if is_media_file:
                ...
                # Check if using new editor controller
                if hasattr(self, 'project'): # Use self.project as indicator for new architecture
                    # Use new project loading
                    if self.load_project(file_path):
                        self._current_project_file = file_path
                    ...
                    # Fallback to old pattern loading
    ...
    def force_dimensions_override(self):
        ...
        try:
            if not self.repository.get_current_file() or not os.path.exists(self.repository.get_current_file()):
    ...
    def _save_project_common(self, file_path: Optional[str] = None) -> bool:
        """Common logic for saving a project or pattern."""
        if hasattr(self, 'project') and self.project:
            ...
            try:
                # Use the real save_project implementation
                if self.save_project(file_path): 
                    self._current_project_file = file_path
                    base_name = os.path.basename(file_path)
                    self.setWindowTitle(f"Upload Bridge - {base_name}")
                    self.status_bar.showMessage(f"Saved project: {base_name}")
            ...
            if not file_path or not file_path.endswith('.ledproj'):
                file_path, _ = QFileDialog.getSaveFileName(
                    self,
                    "Save Project",
                    self.settings.value("last_directory", ""),
    ...
    def save_project_menu_action(self):
        """Save current project - called from menu action (no parameters)"""
        # Check if using new editor controller
        if hasattr(self, 'project') and self.project:
            # Use new controller for saving
            current_file = getattr(self, '_current_project_file', None)
            if current_file:
                if self.save_project(current_file):
    ...
    def save_project_as(self):
        ...
        # Check if using new editor controller
        if hasattr(self, 'project') and self.project: # Check self.project
            file_path, _ = QFileDialog.getSaveFileName(
                self,
                "Save Project",
                self.settings.value("last_directory", ""),
            ...
            if self.save_project(file_path): # Call self.save_project
                self._current_project_file = file_path  # Store for future saves
                base_name = os.path.basename(file_path)
                self.setWindowTitle(f"Upload Bridge - {base_name}")
                QMessageBox.information(self, "Saved", "Project saved successfully!")
            return
    ...
    def load_pattern_to_all_tabs(self, pattern: Pattern, file_path: str = None):
        ...
        # Preserve original pattern ID to ensure consistency across tabs
        original_id = pattern.id if hasattr(pattern, 'id') else None
        ...
        # Ensure repository pattern has correct ID
        repo_pattern = self.repository.get_current_pattern()
        if repo_pattern and original_id and hasattr(repo_pattern, 'id') and repo_pattern.id != original_id:
            repo_pattern.id = original_id
        ...
        try:
            ...
            if design_tab and hasattr(design_tab, 'load_pattern'):
                ...
                logging.getLogger(__name__).warning("Design Tools tab does not support load_pattern method")
        except Exception as e:
            error_msg = f"Design Tools: {str(e)}"
            tabs_failed.append(error_msg)
            logging.getLogger(__name__).error(
                "Design Tools tab load failed: %s (pattern: %s, frames: %d)",
                e, pattern.name, pattern.frame_count, exc_info=True
            )
        ...
        try:
            ...
            if flash and hasattr(flash, 'load_pattern'):
                ...
                logging.getLogger(__name__).warning("Flash tab does not support load_pattern method")
        except Exception as e:
            error_msg = f"Flash: {str(e)}"
            tabs_failed.append(error_msg)
            logging.getLogger(__name__).error(
                "Flash tab load failed: %s (pattern: %s, frames: %d)",
                e, pattern.name, pattern.frame_count, exc_info=True
            )
        ...
        try:
            ...
            if batch_flash and hasattr(batch_flash, 'load_pattern'):
                ...
                logging.getLogger(__name__).warning("Batch Flash tab does not support load_pattern method")
        except Exception as e:
            error_msg = f"Batch Flash: {str(e)}"
            tabs_failed.append(error_msg)
            logging.getLogger(__name__).error(
                "Batch Flash tab load failed: %s (pattern: %s, frames: %d)",
                e, pattern.name, pattern.frame_count, exc_info=True
            )
        ...
        try:
            ...
            if wifi and hasattr(wifi, 'set_pattern'):
                ...
                logging.getLogger(__name__).warning("WiFi Upload tab does not support set_pattern method")
        except Exception as e:
            error_msg = f"WiFi Upload: {str(e)}"
            tabs_failed.append(error_msg)
            logging.getLogger(__name__).error(
                "WiFi Upload tab load failed: %s (pattern: %s, frames: %d)",
                e, pattern.name, pattern.frame_count, exc_info=True
            )
        ...
        try:
            ...
            error_msg = f"Pattern Library: {str(e)}"
            tabs_failed.append(error_msg)
            logging.getLogger(__name__).error(
                "Pattern Library tab initialization failed: %s",
                e, exc_info=True
            )
        
        # Report if any tabs failed
        if tabs_failed:
            failed_count = len(tabs_failed)
            total_tabs = len(tabs_loaded) + failed_count
            self.status_bar.showMessage(
                f"Pattern loaded to {len(tabs_loaded)}/{total_tabs} tabs. "
                f"{failed_count} tab(s) failed to load."
            )
            # Only show error dialog if critical tabs failed
            critical_tabs_failed = [f for f in tabs_failed if any(critical in f for critical in ["Preview", "Design Tools"])]
            if critical_tabs_failed:
                QMessageBox.warning(
                    self,
                    "Pattern Load Warning",
                    f"Pattern loaded but some critical tabs failed:\n\n" + "\n".join(critical_tabs_failed[:3]) +
                    (f"\n... and {len(critical_tabs_failed) - 3} more" if len(critical_tabs_failed) > 3 else "")
                )
    ...
    def _on_project_changed(self, project):
        """Handle project changes from editor controller."""
        # Update status bar or other UI elements as needed
        if hasattr(self, 'statusBar'):
            frame_count = project.timeline.total_frames if project else 0
            self.statusBar.showMessage(f"Project: {frame_count} frames")

        # You can add more project change handling here
        # For example, updating preview tabs, etc.
        self.repository.set_dirty(True)  # Mark as dirty when project changes
        self._sync_legacy_attributes()
        # The line below was from media, and might not be applicable here directly.
        # self.status_bar.showMessage(f"Pattern loaded from media: {pattern.name}")
        # Offer to add to library only if it's a newly created/modified pattern that's not part of a loaded project.
        # self._offer_add_to_library(pattern, "media upload")
    
    def show_about(self):
        """Show about dialog"""
        # Get version from app_version module
        try:
            ...
            try:
                ...
                config = get_config()
                version = config.get('app_version', '3.0.0')
            except Exception:
                version = "3.0.0"
        
        about_text = f"""
        <h2>Upload Bridge v{version}</h2>
    ...
    def show_license_activation(self):
        """Show license activation dialog (account-based)."""
        try:
            from core.config import get_config
            ...
            config = get_config()
            server_url = config.get("auth_server_url") or os.environ.get("LICENSE_SERVER_URL") or os.environ.get("AUTH_SERVER_URL") or "http://localhost:8000"
            ...
            dialog.activation_successful.connect(lambda: self.status_bar.showMessage("✅ License activated successfully"))
            dialog.exec()
        except Exception as e:
            QMessageBox.warning(self, "Activation", f"Failed to open activation dialog: {e}")
    
    def show_license_status(self):
        """Show current license status (account-based)."""
        try:
            from core.config import get_config
            from core.auth_manager import AuthManager
            from core.license_manager import LicenseManager
            from ui.dialogs.license_status_dialog import LicenseStatusDialog
            ...
            QMessageBox.warning(self, "License Status", f"Failed to open license status dialog: {e}")

    def deactivate_license(self):
        """Logout and clear license session."""
        try:
            from core.config import get_config
            from core.auth_manager import AuthManager
            from core.license_manager import LicenseManager
            ...
            reply = QMessageBox.question(
                self,
                "Deactivate License",
                "Are you sure you want to logout and deactivate your license?\n\n"
            ...
            if reply == QMessageBox.Yes:
                ...
                # Clear license cache
                if license_manager.cache_file.exists():
                    license_manager.cache_file.unlink()
                if license_manager.encrypted_license_file.exists():
                    license_manager.encrypted_license_file.unlink()
            ...
            QMessageBox.warning(self, "Deactivate", f"Failed to deactivate license: {e}")
    ...
    def show_diagnostic_report(self):
        """Show diagnostic report dialog"""
        try:
            from ui.dialogs.diagnostic_report_dialog import DiagnosticReportDialog
            dialog = DiagnosticReportDialog(self, app_version="3.0.0")
            dialog.exec()
        except Exception as e:
            logging.getLogger(__name__).error(f"Error showing diagnostic report dialog: {e}", exc_info=True)
            QMessageBox.critical(
                self,
                "Error",
                f"Failed to open diagnostic report dialog:\\n{str(e)}"
            )
    ...
    def _on_workspace_active_changed(self, pattern_name: str):
        ...
        if pattern_name:
            ...
            if pattern and pattern != self.repository.get_current_pattern():
                self.load_pattern_to_all_tabs(pattern, None)
