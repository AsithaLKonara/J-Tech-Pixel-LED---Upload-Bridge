// esp_matrix_player_wifi.ino
// ESP8266 LED Matrix Pattern Player with WiFi Upload
...
// - pattern_data.h (generated by GUI application)
...
// Include the pattern data (generated by Python script)
...
<html>
<head>
    <title>LED Matrix Pattern Upload</title>
...
<body>
    <div class="container">
        <h1>🎨 LED Matrix Pattern Upload</h1>
        ...
        <div class="upload-area" onclick="document.getElementById('fileInput').click()">
            <p>📁 Click here to select pattern file (.bin)</p>
        ...
        <div id="file-details" style="display: none;">
            ...
            <button onclick="uploadPattern()">🚀 Upload Pattern</button>
        ...
        <div style="margin-top: 30px; text-align: center;">
            ...
            <button onclick="window.open('http://192.168.4.1/status', '_blank')">📊 Detailed Status</button>
        </div>
    </div>
        ...
        function uploadPattern() {
            ...
            xhr.onload = function() {
                ...
                if (xhr.status === 200) {
                    ...
                    if (response.success) {
                        document.getElementById('upload-result').innerHTML = 
                            '<div class="status success">✅ Pattern uploaded successfully!<br>' + response.message + '</div>';
                        updateStatus();
                    } else {
                        document.getElementById('upload-result').innerHTML = 
                            '<div class="status error">❌ Upload failed: ' + response.message + '</div>';
                    }
                } else {
                    document.getElementById('upload-result').innerHTML = 
                        '<div class="status error">❌ Upload failed with status: ' + xhr.status + '</div>';
                }
            };
...
void handleUpload() {
  ...
  if (upload.status == UPLOAD_FILE_START) {
    ...
    String message = "Pattern loaded: " + String(current_num_leds) + " LEDs × " + String(current_num_frames) + " frames (" + String(current_pattern_size) + " bytes)";
...
void setup() {
  Serial.begin(115200);
  delay(100);
  Serial.println("\n\nESP8266 Matrix Pattern Player with WiFi Upload");
  Serial.println("=============================================");
  ...
  if (pattern_data_size > 0) {
    Serial.printf("✓ Default pattern loaded: %u bytes\n", pattern_data_size);
    
    // Parse default pattern header
    uint16_t num_leds = read_u16_pgm(pattern_data, 0);
    uint16_t num_frames = read_u16_pgm(pattern_data, 2);
    
    Serial.printf("Default pattern: %u LEDs × %u frames\n", num_leds, num_frames);
  } else {
    Serial.println("⚠ No default pattern data found");
  }
