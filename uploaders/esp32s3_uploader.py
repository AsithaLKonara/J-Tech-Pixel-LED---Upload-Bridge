"""
ESP32-S3 Uploader Adapter - Standard UploaderAdapter implementation for ESP32-S3

Implements the UploaderAdapter interface for ESP32-S3 chips using esptool.
"""
...
class ESP32S3Uploader(UploaderAdapter):
    """Uploader adapter for ESP32-S3 chips"""
    ...
    def detect_device(self, port: Optional[str] = None) -> Optional[DeviceInfo]:
        ...
        try:
            ...
            if result.returncode == 0 and "ESP32-S3" in result.stdout:
                return DeviceInfo(
                    chip_id="ESP32",
                    chip_variant="ESP32-S3",
                    port=port or "auto",
    ...
    def build_firmware(
        ...
        """Build ESP32-S3 firmware from pattern"""
        try:
    ...
    def flash_firmware(
        ...
        try:
            cmd = [
                ...
                "--flash_size", options.flash_size or "8MB",
    ...
    def get_device_profile(self) -> Dict[str, Any]:
        """Get ESP32-S3 device profile"""
        profile_path = Path(__file__).parent / "profiles" / "esp32s3.json"
        ...
        return {
            ...
            "capabilities": ["flash", "flash", "verify", "erase"]
        }


register_adapter(ESP32S3Uploader)
