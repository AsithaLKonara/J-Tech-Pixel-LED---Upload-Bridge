/**
 * Professional Nuvoton M031 LED Pattern Player - Upload Bridge Template
 * 
 * Full professional-grade pattern player with all advanced features
 * Optimized for NuMicro M031 with 64KB flash and 8KB RAM
...
#define MAX_LEDS 200  // Optimized for NuMicro M031 memory
...
// Speed keyframes (max 12 keyframes for NuMicro M031)
struct SpeedKeyframe {
    uint16_t frame;
    float multiplier;
};
SpeedKeyframe speed_keyframes[12];
...
int main(void) {
    ...
    for (uint8_t i = 0; i < num_keyframes && i < 12; i++) {
...
float apply_brightness_curve(float value, uint8_t curve_type) {
    ...
    switch (curve_type) {
        ...
        case BRIGHTNESS_CURVE_GAMMA_CORRECTED:
            // Full gamma correction for NuMicro M031
...
void system_clock_config(void) {
    // Enable HXT (external crystal)
    CLK->PWRCTL |= CLK_PWRCTL_HXTEN;
    while (!(CLK->STATUS & CLK_STATUS_HXTSTB));
    
    // Configure PLL for 48MHz
    CLK->PLLCTL = CLK_PLLCTL_48MHz_HXT;
    while (!(CLK->STATUS & CLK_STATUS_PLLSTB));
...
void ws2812_init(uint8_t pin) {
    // Enable GPIO clock
    CLK->AHBCLK |= CLK_AHBCLK_GPIOAEN;
    
    // Configure pin as output
    GPIOA->MODE = (GPIOA->MODE & ~(0x3 << (pin * 2))) | (0x1 << (pin * 2));
}

void ws2812_send_byte(uint8_t byte) {
    // NuMicro M031 WS2812 bit-banging implementation (optimized for 48MHz)
    for (uint8_t i = 0; i < 8; i++) {
        if (byte & 0x80) {
            // Send 1 (0.9us high, 0.35us low)
            GPIOA->DOUT |= (1 << DATA_PIN);
            for (volatile int j = 0; j < 8; j++); // ~0.9us at 48MHz
            GPIOA->DOUT &= ~(1 << DATA_PIN);
            for (volatile int j = 0; j < 3; j++);  // ~0.35us at 48MHz
        } else {
...
void ws2812_send(uint8_t *data, uint16_t len) {
    ...
    // Reset pulse
    GPIOA->DOUT &= ~(1 << DATA_PIN);
    for (volatile int i = 0; i < 300; i++); // ~50us at 48MHz
}

void delay_ms(uint32_t ms) {
    // Simple delay loop for NuMicro M031 at 48MHz
    for (uint32_t i = 0; i < ms; i++) {
        for (volatile int j = 0; j < 4800; j++); // ~1ms at 48MHz
    }
}
